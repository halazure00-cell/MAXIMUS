            ...
            <Card className="w-full max-w-md p-8 shadow-ui-lg">
                <div className="text-center mb-8">
                    <div className="w-16 h-16 bg-ui-primary-soft rounded-full flex items-center justify-center mx-auto mb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-ui-text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {!isSupabaseConfigured && (
                    <div className="bg-ui-warning/10 border border-ui-warning/30 text-ui-warning px-4 py-3 rounded-ui-md relative mb-4" role="alert">
                ...
                {message ? (
                    <div className="bg-ui-success/10 border border-ui-success/30 text-ui-success px-4 py-3 rounded-ui-md relative mb-4" role="alert">
                        <span className="block sm:inline">{message}</span>
                    </div>
                ) : (
                    <form onSubmit={handleLogin} className="space-y-4">
                        {errorMessage ? (
                            <div className="bg-ui-danger/10 border border-ui-danger/30 text-ui-danger px-4 py-3 rounded-ui-md relative" role="alert">
                                <span className="block sm:inline">{errorMessage}</span>
                            </div>
                        ) : null}
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium text-ui-text mb-1">Email</label>
                            <input
                                id="email"
                        ...
                        <PrimaryButton
                            className="w-full py-3 text-base font-bold shadow-ui-md"
                            disabled={loading || !isSupabaseConfigured}
                        >
                            {loading ? (
                                <span className="flex items-center justify-center">
                                    <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-ui-text" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <p className="mt-8 text-center text-xs text-ui-muted">
                    Powered by Supabase
                </p>
            </Card>
        </div>
